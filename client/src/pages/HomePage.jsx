...
import { formatAmount, priceClass, formatSign } from '../utils/indicators.js';
  ...
  return (
    <div>
      {indices.length > 0 && (
        <div className="indices-bar">
          {indices.map((idx) => {
            const cls = priceClass(idx.change);
            return (
              <div key={idx.symbol} className={`index-card ${cls}`} onClick={() => navigate(`/stock/${idx.symbol}`)}>
                ...
                <div className={`index-change ${cls}`}>
                  <span>{formatSign(idx.change)}{idx.change?.toFixed(2)}</span>
                  <span>{formatSign(idx.changePercent)}{idx.changePercent?.toFixed(2)}%</span>
                </div>
